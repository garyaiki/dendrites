...
class DecayedValueSpec extends FlatSpecLike with TrigUtils {
  ...
  it should "be less than the mean at 180º" in { assert(decayedValuesten(180).average(10.0) < meanDay180) }

  it should "be less than the mean at 270º" in { assert(decayedValuesten(270).average(10.0) < meanDay270) }

  it should "be less than mean at 360º" in { assert(decayedValuesten(360).average(10.0) < meanDay360) }
  ...
  it should "exceed the decayed average at 180º" in {
    assert(decayedValuesten(180).averageFrom(10.0, 170.0, 180.0) > decayedValuesten(180).average(10.0))
  }

  it should "be less than the decayed average at 270º" in {
    assert(decayedValuesten(270).averageFrom(10.0, 260.0, 270.0) < decayedValuesten(270).average(10.0))
  }

  it should "be less than the decayed average at 360º" in {
    assert(decayedValuesten(360).averageFrom(10.0, 350.0, 360.0) < decayedValuesten(360).average(10.0))
  }
