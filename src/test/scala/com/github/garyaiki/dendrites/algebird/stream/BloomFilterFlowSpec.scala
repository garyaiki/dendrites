...
class BloomFilterFlowSpec extends WordSpecLike with BloomFilterBuilder {
  ...
  val properFilter: Flow[String, String, NotUsed] = Flow[String].filter { x => properBF.contains(x).isTrue }
  ...
  val connectivesFilter: Flow[String, String, NotUsed] = Flow[String].filter { x => connectivesBF.contains(x).isTrue }
  ...
  val wordsFilter: Flow[String, String, NotUsed] = Flow[String].filter { x => wordsBF.contains(x).isTrue }
