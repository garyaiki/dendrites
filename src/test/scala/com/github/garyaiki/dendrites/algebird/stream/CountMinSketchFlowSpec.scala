...
class CountMinSketchFlowSpec extends WordSpecLike with InetAddressesBuilder {
  ...
  val longs: Flow[IndexedSeq[(Long, Int)], IndexedSeq[Long], NotUsed] = Flow[IndexedSeq[(Long, Int)]].map(testLongs)
  ...
  val cmSketch: Flow[IndexedSeq[Long], CMS[Long], NotUsed] = Flow[IndexedSeq[Long]].map(createCountMinSketch[Long])
  val (pub, sub) = TestSource.probe[IndexedSeq[Long]].via(cmSketch).toMat(TestSink.probe[CMS[Long]])(Keep.both).run()
  sub.request(1)
  ...
  it should {
    "sum total count over a Sequence of them" in {
      ...
      val cmsSummer: Flow[Seq[CMS[Long]], CMS[Long], NotUsed] = Flow[Seq[CMS[Long]]].map(sumCountMinSketch[Long])
      val (pub, sub) = TestSource.probe[Seq[CMS[Long]]].via(cmsSummer).toMat(TestSink.probe[CMS[Long]])(Keep.both).run()
      sub.request(1)
      pub.sendNext(cmss)
