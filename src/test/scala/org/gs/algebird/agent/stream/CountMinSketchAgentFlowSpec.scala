...
class CountMinSketchAgentFlowSpec extends FlatSpecLike with TestValuesBuilder {
  ...
  "A composite CountMinSketchAgentFlow of Longs" should "update its total count" in {
    val cmsAgt = new CountMinSketchAgent[Long]("test Longs")
  ...
  "A composite sink CountMinSketchAgentFlow of Longs" should "update its total count" in {
    ...
    whenReady(updateFuture, timeout) { result =>
      result.totalCount should equal(longs.size)  //@FIXME
    }
  }
}
