...
[Custom stages](http://doc.akka.io/docs/akka/current/scala/stream/stream-customize.html){:target="_blank"} are necessary to process functions with contextual information. Some need the streams' lifecycle. Others need state. Logging is vital to knowing what happens in asynchronous code, handling results in-stage allows log messages to have input, output, and context information. Errors in distributed systems are often temporary, several *dendrites* stages retry these with [exponential backoff](https://en.wikipedia.org/wiki/Exponential_backoff){:target="_blank"}. Other errors have enhanced error and log messages. Kafka polling and Cassandra queries return collections, custom stages input these and output a specified number of elements when pulled from downstream.
