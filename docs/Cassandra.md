...
[Cassandra](https://academy.datastax.com/planet-cassandra/what-is-apache-cassandra){:target="_blank"} is one of the most popular distributed databases. *dendrites* generalizes many frequently used Cassandra operations, including backpressure to prevent overflow and local error handling with recovery. Pre-built Cassandra stream stages wrap Datastax’s Cassandra [Java Driver](http://docs.datastax.com/en/developer/java-driver//3.1/){:target="_blank"} and call it asynchronously.

##### Setup Cassandra client

Before running streams, the Cassandra cluster connection is configured with a [CassandraConfig](https://github.com/garyaiki/dendrites/blob/master/src/main/scala/com/github/garyaiki/dendrites/cassandra/CassandraConfig.scala){:target="_blank"}, initialized and managed with [provided functions](https://github.com/garyaiki/dendrites/blob/master/src/main/scala/com/github/garyaiki/dendrites/cassandra/package.scala){:target="_blank"}, and [PreparedStatements](http://docs.datastax.com/en/developer/java-driver//3.1/manual/statements/prepared/){:target="_blank"} are pre-parsed in the database.
```scala
...
##### Cassandra insert, delete
[CassandraSink](https://github.com/garyaiki/dendrites/blob/master/src/main/scala/com/github/garyaiki/dendrites/cassandra/stream/CassandraSink.scala){:target="_blank"} executes delete and insert BoundStatements that don’t return a useful ResultSet. It also wraps the Java driver and calls it asynchronously.
